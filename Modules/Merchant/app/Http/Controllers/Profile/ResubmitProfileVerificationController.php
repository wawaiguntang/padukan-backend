...
use Modules\Merchant\Http\Requests\ProfileVerificationRequest;
...
class ResubmitProfileVerificationController
{
    ...
    public function __invoke(ProfileVerificationRequest $request): JsonResponse
    {
        ...
        try {
            ...
            if ($profile->verification_status === \Modules\Merchant\Enums\VerificationStatusEnum::REJECTED) {
                ...
                return response()->json([
                    'status' => true,
                    'message' => __('merchant::controller.profile.verification.resubmitted_successfully'),
                    'data' => $result,
                ]);
            } else {
                return response()->json([
                    'status' => false,
                    'message' => __('merchant::controller.profile.verification.resubmit_not_allowed'),
                ], 400);
            }
        } catch (\Exception $e) {
            return response()->json([
                'status' => false,
                'message' => __('merchant::controller.profile.verification.resubmission_failed'),
                'error' => $e->getMessage(),
            ], 500);
        }
    }
