...
class SubmitProfileVerificationController
{
    ...
    public function __invoke(ProfileVerificationRequest $request): JsonResponse
    {
        ...
        try {
            if ($profile->verification_status === \Modules\Merchant\Enums\VerificationStatusEnum::PENDING) {
                ...
                return response()->json([
                    'status' => true,
                    'message' => __('merchant::controller.profile.verification.submitted_successfully'),
                    'data' => $result,
                ], 201);
            } else {
                return response()->json([
                    'status' => false,
                    'message' => __('merchant::controller.profile.verification.cannot_submit'),
                ], 400);
            }
        } catch (\Exception $e) {
            return response()->json([
                'status' => false,
                'message' => __('merchant::controller.profile.verification.submission_failed'),
                'error' => $e->getMessage(),
            ], 500);
        }
    }
