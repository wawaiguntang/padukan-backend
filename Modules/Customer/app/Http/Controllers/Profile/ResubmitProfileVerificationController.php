...
class ResubmitProfileVerificationController
{
    ...
    public function __invoke(ProfileVerificationRequest $request): JsonResponse
    {
        ...
        try {
            ...
            if ($profile->verification_status === \Modules\Customer\Enums\VerificationStatusEnum::REJECTED) {
                ...
                return response()->json([
                    'status' => true,
                    'message' => __('customer::controller.profile.verification.resubmitted_successfully'),
                    'data' => $result,
                ]);
            } else {
                return response()->json([
                    'status' => false,
                    'message' => __('customer::controller.profile.verification.resubmit_not_allowed'),
                ], 400);
            }
        } catch (\Exception $e) {
            return response()->json([
                'status' => false,
                'message' => __('customer::controller.profile.verification.resubmission_failed'),
                'error' => $e->getMessage(),
            ], 500);
        }
    }
